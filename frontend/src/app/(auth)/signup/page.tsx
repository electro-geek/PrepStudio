    ...
    <div className="relative min-h-screen bg-paper text-ink flex items-center justify-center px-4 py-16 blueprint">
      ...
      <div className="relative z-10 w-full max-w-[440px]">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="macro text-2xl">CREATE ACCOUNT</h2>
          <p className="mono-label-sm text-ink-500 mt-2">[ NEW OPERATOR INTAKE ]</p>
        </div>
          ...
          <div className="p-7">
            {isMock && (
              <div className="border-2 border-ink bg-paper-alt px-3.5 py-3 mb-5 flex items-start gap-2.5 text-xs">
                ...
                <span className="text-ink-700"><strong className="font-mono uppercase">DEV MODE:</strong> Mock authentication active. Any credentials work.</span>
              </div>
            )}
            {error && (
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              <div>
                <label className="mono-label-sm text-ink-500 block mb-2">FULL NAME <span className="text-ink-400">(OPTIONAL)</span></label>
                <div className="relative">
                  <User className="absolute left-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-ink-400" />
                  <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="John Doe"
                         className="field-ind w-full pl-10 pr-4 py-3 text-sm" />
                </div>
              </div>
              <div>
        ...
        <p className="text-center mono-label-sm text-ink-500 mt-4">
          HAVE AN ACCOUNT?{" "}
          <Link href="/login" className="text-hazard hover:text-ink font-semibold">SIGN IN →</Link>
        </p>
      </div>
    </div>
  );
